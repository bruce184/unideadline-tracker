...
function isEmailReminderConfigured() {
  return process.env.EMAIL_REMINDER_ENABLED === 'true'
...
export async function processEmailReminders() {
  if (!isEmailReminderConfigured()) {
    console.warn('[EmailReminder] Email reminders are disabled or SMTP env is incomplete')
    return { processed: 0, sent: 0, failed: 0, skipped: 'email_disabled' }
  }
...
export function startEmailReminderCron() {
  if (!isEmailReminderConfigured()) {
    console.log('[EmailReminder] Cron disabled. Set EMAIL_REMINDER_ENABLED=true and SMTP env to enable it.')
    return null
  }
